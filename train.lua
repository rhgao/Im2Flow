...
opt = {
   ...
   niter = 200,            -- #  of iter from starting learning rate
   ...
   display_plot = 'errL2', -- which loss values to plot over time. Accepted values include a comma seperated list of: errL2, errG, and errD
   ...
   decay_epoch_freq = 100,      -- decay the learning rate by half  every decay_epoch_freq
   ...
   display_freq = 100,          -- display the current rcesults every display_freq iterations
   ...
   use_ContentLoss = 1,               -- set to 0 to turn off content loss
   content_loss = 'resnet18',         -- which model to get content loss, only resnet18 supported
   layer_name = '',                   -- which layer to compute content loss
   which_model_netD = 'basic', -- selects model to use for netD
   which_model_netG = 'unet',  -- selects model to use for netG
   n_layers_D = 0,             -- only used if which_model_netD=='n_layers'
   lambda_L2 = 100,            -- weight on L2 term in objective
   lambda_ContentLoss = 100,   -- weight on Content Loss term in objective
}
...
function defineD(input_nc, output_nc, ndf)
    ...
    if     opt.which_model_netD == "basic" then netD = defineD_basic(3, 3, ndf)
    elseif opt.which_model_netD == "n_layers" then netD = defineD_n_layers(3, 3, ndf, opt.n_layers_D)
...
local errD, errG, errL2, errContent = 0, 0, 0, 0
...
if opt.gpu > 0 then
   ...
   netD:cuda(); netG:cuda(); criterion:cuda(); criterionMSE:cuda(); criterionContent:cuda();
   print('done')
else
	print('running model on CPU')
end

if opt.use_ContentLoss == 1 then
  contentFunc = content.defineContent(opt.content_loss, opt.layer_name, opt)
end
...
local fGx = function(x)
    ...
    -- L2 loss
    local df_do_MSE = torch.zeros(fake_B:size())
    if opt.gpu>0 then 
      df_do_MSE = df_do_MSE:cuda()
    end
    if opt.use_L2==1 then
       errL2 = criterionMSE:forward(fake_B, real_B)
       df_do_MSE = criterionMSE:backward(fake_B, real_B)
    else
       errL2 = 0
    end 

    -- Content loss
    local df_d_content = torch.zeros(fake_B:size())
    if opt.use_ContentLoss == 1 then
      errContent, df_d_content = content.lossUpdate(criterionContent, real_B, fake_B, contentFunc, opt.content_loss, opt.lambda_ContentLoss)
    end


    -- weights for gradients
    local gradients_weight = torch.ones(fake_B:size())
    if opt.gpu>0 then
      gradients_weight = gradients_weight:cuda()
    end
    gradients_weight[{{},{1},{},{}}]:copy((real_B[{{},{3},{},{}}] + 1) / 2) ----- [-1,1] to [0,1]
    gradients_weight[{{},{2},{},{}}]:copy((real_B[{{},{3},{},{}}] + 1) / 2) ----- [-1,1] to [0,1]

    --netG:forward(real_A)
    netG:backward(real_A, torch.cmul(df_dg + df_do_MSE:mul(opt.lambda_L2) + df_d_content, gradients_weight))
...
for k, v in ipairs(opt.display_plot) do
    if not util.containsValue({"errG", "errD", "errL2"}, v) then 
...
for epoch = 1, opt.niter do
    ...
    for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
        ...
        -- (2) Update G network: maximize log(D(x,G(x))) + L2(y,G(x))
        ...
        if counter % opt.save_display_freq == 0 and opt.display then
            ...
            print(('saving the current model (epoch %d, iters %d)'):format(epoch, counter))
            --torch.save(paths.concat(opt.checkpoints_dir, opt.name, 'net_G_' .. counter .. '.t7'), netG:clearState())
            --torch.save(paths.concat(opt.checkpoints_dir, opt.name, 'net_D_' .. counter .. '.t7'), netD:clearState())
            opt.serial_batches=serial_batches
        end
        
        -- logging and display plot
        if counter % opt.print_freq == 0 then
            local loss = {errG=errG and errG or -1, errD=errD and errD or -1, errL2=errL2 and errL2 or -1, errContent=errContent and errContent or -1}
            local curItInBatch = ((i-1) / opt.batchSize)
            local totalItInBatch = math.floor(math.min(data:size(), opt.ntrain) / opt.batchSize)
            print(('Epoch: [%d][%8d / %8d]\t Time: %.3f  DataTime: %.3f  '
                    .. '  Err_G: %.4f  Err_D: %.4f ErrL2: %.4f errContent: %.4f'):format(
                     epoch, curItInBatch, totalItInBatch,
                     tm:time().real / opt.batchSize, data_tm:time().real / opt.batchSize,
                     errG, errD, errL2, errContent))
    ...
    if epoch % opt.decay_epoch_freq == 0 then
	     opt.lr = opt.lr * 0.5
    end
